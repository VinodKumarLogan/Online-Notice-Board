...
{
var agree=confirm("Are you sure that you want to delete the selected notifications?");
if (agree)
{
...
$result = mysql_query ("SELECT noti_id,notification,noti_type FROM onb_notifications");
echo <<<disp
<form action="delnoti.php" method="post" name="list" onsubmit="return confirmDelete()" >
<table width="100%">
<tr>
    <td><b>ID</b></td>
...
$count_id=0;
     ...
     {	$temp_id=$row['noti_id'];
	$temp_noti=$row['notification'];
	$temp_type=$row['noti_type'];
	if($temp_type==0)
		$temp_type="Public";
	else if($temp_type==9)
		$temp_type="All";
	...
	<td>{$temp_noti}</td>
	<td>{$temp_type}</td>
		     <td><input type='checkbox' name="$temp_id" /></td></tr>
disp;
      }
      echo "</table>";
      echo <<<disp
